...
class DetectScam(commands.Cog):
    ...
    async def ticket_with_link(self, message):
        ...
        if any(x in txt for x in ["ticket", "support team", "admin", "mod"]) and "http" in txt:
    ...
    async def paperhands(self, message):
        ...
        if (any(x in txt for x in ["paperhand", "paperhold", "pages.dev"]) and "http" in txt) or "pages.dev" in txt:
