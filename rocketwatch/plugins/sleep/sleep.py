...
class Oura(commands.Cog):
    ...
    async def sleep_schedule(self, ctx: Context):
        ...
        # plot
        fig, ax = plt.subplots()
        # create horizontal dark gray line at 0am across the entire graph
        ...
        ax.set_xticks(range(len(daily_sleep), 0, -7))
        ax.set_xticklabels([day for i, (day, _) in enumerate(reversed(daily_sleep.items())) if i % 7 == 0])
        # set y axis labels
